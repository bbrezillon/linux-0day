					   ...
					   int partno, uint64_t cur_offset)
{
	...
	child->props.part.flags = parent->flags & ~part->mask_flags;
	child->flags = child->props.part.flags;
	...
	child->parent = parent;
	child->props.part.offset = part->offset;
	INIT_LIST_HEAD(&child->partitions);

	if (child->props.part.offset == MTDPART_OFS_APPEND)
		child->props.part.offset = cur_offset;
	if (child->props.part.offset == MTDPART_OFS_NXTBLK) {
		tmp = cur_offset;
		child->props.part.offset = cur_offset;
		remainder = do_div(tmp, wr_alignment);
		if (remainder) {
			child->props.part.offset += wr_alignment - remainder;
			printk(KERN_NOTICE "Moving partition %d: "
			       "0x%012llx -> 0x%012llx\n", partno,
			       (unsigned long long)cur_offset,
			       child->props.part.offset);
		}
	}
	if (child->props.part.offset == MTDPART_OFS_RETAIN) {
		child->props.part.offset = cur_offset;
		if (parent->size - child->props.part.offset >= child->size) {
			child->size = parent->size - child->props.part.offset -
				      child->size;
		} else {
			printk(KERN_ERR "mtd partition \"%s\" doesn't have enough space: %#llx < %#llx, disabled\n",
			       part->name,
			       parent->size - child->props.part.offset,
			       child->size);
	...
	printk(KERN_NOTICE "0x%012llx-0x%012llx : \"%s\"\n",
	       child->props.part.offset, child->props.part.offset + child->size,
	       child->name);

	/* let's do some sanity checks */
	if (child->props.part.offset >= parent->size) {
		/* let's register it anyway to preserve ordering */
		child->props.part.offset = 0;
		child->size = 0;
		printk(KERN_ERR"mtd: partition \"%s\" is out of reach -- disabled\n",
			part->name);
		goto out_register;
	}
	if (child->props.part.offset + child->size > parent->size) {
		child->size = parent->size - child->props.part.offset;
	...
	if (parent->numeraseregions > 1) {
		...
		u64 end = child->props.part.offset + child->size;
		struct mtd_erase_region_info *regions = parent->eraseregions;
		u64 part_offset = child->props.part.offset;

		/* Find the first erase regions which is part of this
		 * partition. */
		for (i = 0; i < max && regions[i].offset <= part_offset; i++)
			;
	...
	tmp = mtd_get_master_offset(child, 0);
	remainder = do_div(tmp, wr_alignment);
	...
	tmp = mtd_get_master_offset(child, 0) + child->size;
	remainder = do_div(tmp, wr_alignment);
	if ((child->flags & MTD_WRITEABLE) && remainder) {
		child->flags &= ~MTD_WRITEABLE;
 ...
 */
static int mtd_del_partition_locked(struct mtd_info *mtd)
{
 ...
 */
static int del_mtd_partitions_locked(struct mtd_info *mtd)
{
	struct mtd_info *child, *next;
	int ret, err = 0;
...
int del_mtd_partitions(struct mtd_info *mtd)
{
	...
	mutex_lock(&master->props.master.partitions_lock);
	ret = del_mtd_partitions_locked(mtd);
	mutex_unlock(&master->props.master.partitions_lock);
...
int mtd_del_partition(struct mtd_info *mtd, int partno)
{
	...
	mutex_lock(&master->props.master.partitions_lock);
	list_for_each_entry(child, &mtd->partitions, props.part.node) {
		if (child->index == partno) {
			ret = mtd_del_partition_locked(child);
			break;
		}
	}
	mutex_unlock(&master->props.master.partitions_lock);
...
u64 mtd_get_device_size(struct mtd_info *mtd)
{
