...
static void mtk_nfc_select_die(struct nand_chip *nand, unsigned int cs)
{
	struct mtk_nfc *nfc = nand_get_controller_data(nand);
	struct mtk_nfc_nand_chip *mtk_nand = to_mtk_nand(nand);

	if (WARN_ON_ONCE(cs >= mtk_nand->nsels))
		return;
			      ...
			      const u8 *buf, int page, int raw)
{
	...
	mtk_nfc_select_die(chip, chip->cur_cs);
