...
static const struct rkvdec_coded_fmt_desc rkvdec_coded_fmts[0];
...
static irqreturn_t rkvdec_irq_handler(int irq, void *priv)
{
	...
	dev_dbg(rkvdec->dev, "dec status %x err status %x\n",
		status, readl(rkvdec->regs + RKVDEC_REG_STRMD_ERR_STA));

	writel(0, rkvdec->regs + RKVDEC_REG_INTERRUPT);

	if (status & RKVDEC_ERR_STA) {
		writel(0, rkvdec->regs + RKVDEC_REG_STRMD_ERR_EN);
		writel(0, rkvdec->regs + RKVDEC_REG_SYSCTRL);
		writel(RKVDEC_SOFTRST_EN_P, rkvdec->regs + RKVDEC_REG_INTERRUPT);
		WARN_ON(readl_poll_timeout(rkvdec->regs + RKVDEC_REG_INTERRUPT,
				   status, status & RKVDEC_SOFTRESET_RDY, 10, 1000000));
	}
