		...
		const struct v4l2_h264_dpb_entry *dpb)
{
	...
	if (!(slice_params->flags & V4L2_H264_SLICE_FLAG_FIELD_PIC))
		b->cur_pic_order_count = min(dec_params->top_field_order_cnt,
	...
	for (i = 0; i < 16; i++) {
		...
		if (!(dpb[i].flags & V4L2_H264_DPB_ENTRY_FLAG_FIELD))
			b->refs[i].pic_order_count = min(dpb[i].top_field_order_cnt,
							 dpb[i].bottom_field_order_cnt);
		else if (dpb[i].flags & V4L2_H264_DPB_ENTRY_FLAG_BOTTOM_FIELD)
			b->refs[i].pic_order_count = dpb[i].bottom_field_order_cnt;
		else
			b->refs[i].pic_order_count = dpb[i].top_field_order_cnt;
